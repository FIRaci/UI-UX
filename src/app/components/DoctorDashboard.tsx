      ...
      {active === "overview" && (
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
          <div className="space-y-5">
            <Card className="p-0 overflow-hidden border-rose-300 bg-gradient-to-br from-rose-50 to-orange-50">
              <div className="p-5">
                <div className="flex items-start gap-3">
                  <div className="w-11 h-11 rounded-xl bg-rose-500 text-white flex items-center justify-center shrink-0 animate-pulse">
                    <AlertTriangle className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Badge className="bg-rose-500 text-white border-0">CẢNH BÁO KHẨN CẤP</Badge>
                      <span className="text-xs text-rose-700">vừa xong</span>
                    </div>
                    <div className="mt-2 tracking-tight">
                      {URGENT_ALERT.patient} <span className="text-muted-foreground">({URGENT_ALERT.age} tuổi)</span>
                    </div>
                    <div className="text-sm text-rose-700 mt-0.5">{URGENT_ALERT.symptoms}</div>
                    <div className="text-xs text-muted-foreground mt-1">AI sàng lọc: {URGENT_ALERT.trigger}</div>
                  </div>
                  <Button
            ...
            <Card className="p-5">
              <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
                <div>
                  <h4 className="tracking-tight">Danh sách ca chờ</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Sắp xếp theo mức độ ưu tiên</p>
                </div>
                <div className="flex gap-2 items-center">
                  ...
                  <Badge variant="secondary">{filteredQueue.length} ca</Badge>
                </div>
              </div>

              {filteredQueue.length === 0 ? (
                <div className="py-12 text-center text-muted-foreground">Không có ca nào trong danh sách chờ.</div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-xs text-muted-foreground">
                      <tr className="text-left">
                        ...
                        <th className="px-3 py-2.5 font-medium">Chờ</th>
                    ...
                    <tbody>
                      {filteredQueue.map(t => (
                        <tr key={t.id} className="border-t hover:bg-slate-50 transition">
                          <td className="px-5 py-3"><LevelBadge level={t.level} /></td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2.5">
                              <Avatar className="w-8 h-8"><AvatarFallback className="bg-violet-100 text-violet-700 text-xs">{t.patient[0]}</AvatarFallback></Avatar>
                              <div>
                                <div>{t.patient}</div>
                                <div className="text-xs text-muted-foreground">{t.age} tuổi • HA {t.vitals.bp}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-3 py-3 text-muted-foreground max-w-xs truncate">{t.symptoms}</td>
                          <td className="px-3 py-3"><span className="inline-flex items-center gap-1 text-muted-foreground"><Clock className="w-3.5 h-3.5" />{t.waited}</span></td>
                          <td className="px-5 py-3 text-right">
                            <Button size="sm" className="bg-slate-900 hover:bg-slate-800" onClick={() => openConsult(t)}>Mở bệnh án</Button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <Card key={p.id} className="p-3 hover:shadow transition">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {p.time} • {p.clinic}
                    </div>
                  </div>
                  <Button
                    size="sm"
