...
export function Appointments({ appointments, onCancel, onEdit, onClickDoctor }: {
      ...
      <Tabs defaultValue="upcoming" className="w-full">
        <div className="border-b border-slate-100 px-5 pt-4 bg-transparent">
          <TabsList className="bg-transparent/40 backdrop-blur-md rounded-xl p-1 h-10 border border-slate-200/50">
            <TabsTrigger value="upcoming" className="rounded-lg text-xs font-bold px-4 py-1.5 data-[state=active]:bg-transparent data-[state=active]:text-slate-800 data-[state=active]:shadow-sm">Sắp tới</TabsTrigger>
            <TabsTrigger value="past" className="rounded-lg text-xs font-bold px-4 py-1.5 data-[state=active]:bg-transparent data-[state=active]:text-slate-800 data-[state=active]:shadow-sm">Đã khám</TabsTrigger>
            <TabsTrigger value="cancelled" className="rounded-lg text-xs font-bold px-4 py-1.5 data-[state=active]:bg-transparent data-[state=active]:text-slate-800 data-[state=active]:shadow-sm">Đã hủy</TabsTrigger>
          </TabsList>
        </div>
        {(["Sắp tới", "Hoàn thành", "Đã hủy"] as const).map((s, i) => (
          <TabsContent key={s} value={["upcoming", "past", "cancelled"][i]} className="p-5 space-y-3.5 m-0 bg-transparent backdrop-blur-xl">
            {appointments.filter(a => a.status === s).length === 0 ? (
              ...
              <div key={a.id} className="flex items-center justify-between p-4.5 border border-white/50 bg-transparent/60 backdrop-blur-md rounded-xl hover:shadow-[0_8px_32px_rgba(0,0,0,0.04)] hover:shadow-emerald-500/10 transition-all card-hover cursor-pointer" style={{ borderRadius: "16px" }} onClick={() => onClickAppt?.(a)}>
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div 
                      ...
                      onClick={(e) => { e.stopPropagation(); onClickDoctor?.(a.doctorName); }}
                    >
                      {a.doctorName}
                    </div>
                ...
                {s === "Sắp tới" && (
                  <div className="flex gap-3 shrink-0">
                    <Button size="sm" variant="outline" className="h-8 rounded-xl text-xs px-3 border-slate-200 text-slate-700 hover:bg-slate-50" onClick={(e) => { e.stopPropagation(); onEdit(a); }}><Pencil className="w-3.5 h-3.5 mr-1 text-slate-400" />Sửa</Button>
                    <Button size="sm" variant="outline" className="h-8 rounded-xl text-xs px-3 text-rose-600 border-rose-100 hover:bg-rose-50/50" onClick={(e) => { e.stopPropagation(); onCancel(a.id); }}><X className="w-3.5 h-3.5 mr-1" />Hủy</Button>
                  </div>
                )}
